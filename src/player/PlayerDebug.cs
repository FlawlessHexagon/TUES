...
public partial class PlayerDebug : SceneTree
{
    public override void _Initialize()
    {
        ...
        Vector3 vel1 = PlayerKinematics.CalculateVelocity(Vector3.Zero, new PlayerInputState(), 4.3f, 6.0f, 6.5f, 9.8f, 0.1, false, false);
        if (!Mathf.IsEqualApprox(vel1.Y, -0.98f)) GD.PrintErr($"Gravity failed: expected -0.98, got {vel1.Y}");

        // Test 2: Jump (Blocked in air)
        Vector3 vel2 = PlayerKinematics.CalculateVelocity(Vector3.Zero, new PlayerInputState(Vector2.Zero, 0, true, false, false), 4.3f, 6.0f, 6.5f, 9.8f, 0.1, false, false);
        if (!Mathf.IsEqualApprox(vel2.Y, -0.98f)) GD.PrintErr("Air Jump failed.");

        // Test 3: Jump (Success on floor)
        Vector3 vel3 = PlayerKinematics.CalculateVelocity(Vector3.Zero, new PlayerInputState(Vector2.Zero, 0, true, false, false), 4.3f, 6.0f, 6.5f, 9.8f, 0.1, true, false);
        ...
        Vector3 vel4 = PlayerKinematics.CalculateVelocity(Vector3.Zero, new PlayerInputState(new Vector2(0, -1), 0, false, false, false), 4.3f, 6.0f, 6.5f, 9.8f, 0.1, true, false);
        ...
        Vector3 vel5 = PlayerKinematics.CalculateVelocity(Vector3.Zero, new PlayerInputState(new Vector2(0, -1), Mathf.Pi / 2f, false, false, false), 4.3f, 6.0f, 6.5f, 9.8f, 0.1, true, false);
